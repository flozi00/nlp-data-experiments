...
ds = ds.filter(lambda example: "import " not in example["conversations"])
ds = ds.filter(lambda example: "```" not in example["conversations"])
ds = ds.filter(lambda example: "#include" not in example["conversations"])
ds = ds.filter(lambda example: "'''" not in example["conversations"])
ds = ds.filter(lambda example: "</" not in example["conversations"])
ds = ds.filter(lambda example: "||" not in example["conversations"])
ds = ds.filter(lambda example: "translat" not in example["conversations"].lower())
ds = ds.filter(lambda example: "terminal" not in example["conversations"].lower())
ds = ds.filter(lambda example: "//" not in example["conversations"])
ds = ds.filter(lambda example: ";" not in example["conversations"])
ds = ds.filter(lambda example: ":=" not in example["conversations"])
ds = ds.filter(lambda example: "[" not in example["conversations"])
ds = ds.filter(lambda example: "|-" not in example["conversations"])
ds = ds.filter(lambda example: '"""' not in example["conversations"])
ds = ds.filter(lambda example: len(example["conversations"]) > 256)
